...
SolveLPRelaxation(Instance& inst)
{
    try {
        ...
        // Collect edges based on reduced cost
	    ...
	    inst.lp_obj = lpObj;
            ...
            for (int i = 0; i < n; i++) {
                for (int j = i + 1; j < n; j++) {
                    ...
                    if (rc <= RC_LIM * lpObj) skeleton.push_back({ i, j });
                    else remaining_edges.push_back({ i, j });
                }
            }
        }
        ...
        cerr << "============================================\n";
        throw;   // or return empty result if you want to continue
    }
    catch (exception& e) {
...
vector<pair<int, int>>
buildSpanningForest(const vector<vector<int>>& skeleton,
    int n,
...
vector<bitset<BSZ>> generateXORCycles_Block_old(
...
vector<bitset<BSZ>> generateXORCycles_Block2( const vector<bitset<BSZ>>& fundBits, const vector<int>& U_local, const vector<int>& V_local, const vector<vector<int>>& incEdges, int maxEdgesPerCycle)
{
    using namespace std;
...
vector<vector<int>> buildRC_Global(
    const vector<vector<bitset<BSZ>>>& cyclesPerBlock,
    const vector<vector<pair<int, int>>>& blockEdgesAll)
{
    using namespace std;
...
vector<vector<bitset<BSZ>>> generateCyclesForAllBlocks(  const BlockData& blocks,  int maxEdgesPerCycle,  ofstream& log,
    Instance& inst)
{
    using namespace std;
    using namespace chrono;
