...
#include "model.h"   // for Instance, GRB*, etc.
...
#include <unordered_map>   // add this include too

struct LPSkeletonRC {
    std::vector<std::vector<int>> skeleton;
    std::vector<std::vector<int>> remaining_edges;
    std::unordered_map<long long, double> rcMap;
};
...
// Compare RC cycles vs inst.precuts
void CompareCstar(std::vector<std::vector<int>>& RC_cycles_vertices,
    Instance& inst,
    const string& type);
...
std::vector<std::vector<int>> computeFundamentalCycles_Block2(
    const std::vector<std::pair<int, int>>& blockEdges,
    const std::vector<int>& blockVerts,
    const int& maxEdgesPerCycle);

std::vector<std::vector<int>> computeFundamentalCycles_Block_MST_RC(
    const std::vector<std::pair<int, int>>& blockEdges,
    const std::vector<int>& blockVerts,
    const std::unordered_map<long long, double>& rcMap,
    const int& maxEdgesPerCycle);
...
std::vector<std::bitset<BSZ>> generateCyclesForSingleBlock(
    ...
    int                                     blockCompId,
    int                                     maxEdgesPerCycle,
    Instance& inst, const unordered_map<long long, double>& rcMap);
