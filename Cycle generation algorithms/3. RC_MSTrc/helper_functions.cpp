...
void readPrecuts(Instance& inst) {
    string savedCutsPath = "path_to_precuts"; //path to precuts

    string precutPath = savedCutsPath + inst.name + ".txt";
    ifstream file(precutPath);
    if (!file.is_open()) {
        throw runtime_error("Unable to open file " + precutPath);
    }
    ...
    while (getline(file, line)) {
        ...
        while (getline(linestream, segment)) {
            ...
            int number;
            vector<int> segmentSet;

            while (numstream >> number) {
                //cout << number << " ";
                segmentSet.push_back(number);
            }
            //cout << endl;

            if (!segmentSet.empty()) { //add only the ones that are not the optimal solution of the instance
                inst.precuts.push_back(segmentSet);
            }
        }
    }
    return;
...
void printInfo(const string& pathAndFileout, const Instance& inst, const Solution& sol) {
    ...
    cout << sol.LB << endl;
    file << fixed << setprecision(2) << inst.name << "\t"
        << inst.precuts.size()<< "\t"
        << inst.nbMST_cycles << "\t"
        << inst.tMST_cycle_Gen << "\t"
        << inst.nbMSTmatch << "\t"
        << inst.percMSTmatch << "\t"
        << inst.tLPrelaxation << "\t"
        << inst.tBuildingBlocksFromSkeleton << "\t"
        << inst.tCycleXOR << "\t"
        << inst.tGlobalRCMerge << "\t"
        << inst.tAllRC << "\t"
        << inst.nbRC_cycles_NOT_RED << "\t"
        << inst.nbRC_cycles_unique << "\t"
        << inst.nbRCmatch << "\t"
        << inst.percRCmatch << "\t"
        << inst.tMergeAndDedupe << "\t"
        << inst.nbMatch << "\t"
        << inst.percMatch << "\t"
        << sol.timeT
        << endl;
