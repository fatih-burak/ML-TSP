...
SolveLPRelaxation(Instance& inst){
	...
	double lpObj = 0.0;
        if (model.get(GRB_IntAttr_Status) == GRB_OPTIMAL) {
            lpObj = model.get(GRB_DoubleAttr_ObjVal);
	    inst.lpObj = lpObj;
            for (int i = 0; i < n; i++) {
                for (int j = i + 1; j < n; j++) {
        ...
        return { skeleton, remaining_edges, lpObj };
    }
...
BlockData buildBlocksFromSkeleton( const vector<vector<int>>& skeleton,  const vector<int>& component,  int n,  ofstream& log)
{
    using namespace std;
    ...
    Instance& inst)
{
    ...
    for (size_t bi = 0; bi < totalBlocks; ++bi)
    {
        ...
        cout << "Block cycle generation time: "   << fixed << setprecision(4)     << t_sec << " sec\n"         << "Cycles found: " << cycles.size() << "\n";
...
RCCyclesResult buildGlobalRCCycles(
    const BlockData& blocks,
    const vector<vector<bitset<BSZ>>>& allBlockCyclesBits,
    ofstream& log)
{
    using namespace std;
    ...
    for (size_t bi = 0; bi < blocks.blockEdgesAll.size(); ++bi)
    {
        ...
        for (const auto& bs : bitsVec)
        {
            ...
            int cntEdges = (int)bs.count(); //Each bitset cycle has cntEdges = bs.count() edges, and we push two endpoints per edge then remove duplicates, so the raw push count is 2*cntEdges.
...
vector<vector<int>> Find_all_RC_CYCLES(Instance& inst)
{
    ...
    auto [skeleton, rem_RC_ordered, lpObj] = SolveLPRelaxation(inst);
...
set<set<int>> Check_K_MSTCycles_Ordered_AvgCliqueLen(const int& needed, Instance& inst, Solution& sol)
{
    ...
    // 2) Canonicalize + remove duplicates + compute clique-average
    struct ScoredCycle {
        vector<int> verts; // canonical: sorted unique vertices
        double score;           // clique average distance
    };

    // Use hash set to remove duplicates cycles by vertex set
...
vector<vector<int>> CheckAllMSTCycles(Instance& inst) {
    ...
    double threshold = (inst.lpObj / inst.nbCity) * FRAC;

    for (int k = 0; k < nonMSTEdges.size(); k++) {
        double w;
        int u, v;
...
vector<vector<int>> mergeAndDedupe(
...
set<set<int>> Check_K_RCCycles_Ordered_AvgCliqueLen(const int& needed, Instance& inst)
{
    ...
    //   but I still canonicalize below jus to be safe.
    ...
    // We can store timing in a new field but create theswe vars
    // inst.tRC_cycle_Gen = duration_cast<nanoseconds>(endALL - startALL).count() / 1e9;
    // inst.nbRC_cycles_selected = (int)result.size();
