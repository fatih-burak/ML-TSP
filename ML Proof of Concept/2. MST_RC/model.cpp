				...
				for (int i = 0; i < j; i++) {
					if (x[i][j].get(GRB_DoubleAttr_Xn) > 0.95) {
						sol_edges.push_back({ i,j });
    						// <<< PRINT VARIABLE THAT IS 1 >>>
            					//if (poolSol == 0) cout << "\\draw (" << i+1 << ") -- (" << j+1 << ");" << endl;
					}
				}
			}
...
void TSP_DLYD(Instance& inst, Solution& sol) {
	...
	double PERC_PRECUTS = inst.precut_perc; //PERCENTAGE OF ACTUAL TRUE CUTS
       	double PRECUT_MULTIPLIER = inst.precut_mult;
	...
	int needed = round(size_selected * PRECUT_MULTIPLIER);
	cout << "Will try to add in total " << needed << " many cuts!" << endl;
	sol.needed = needed;
	int neededRC = (needed + 1) / 2; 

	//GENERATE RC_CYCLES OF SIZE size_selected*PRECUT_MULTIPLIER/2 (MANY CUTS).
	set<set<int>> RCgenSECS = Check_K_RCCycles_Random(neededRC , inst);

	int b4_cuts = sol.cuts.size();
	for (set<int> an_RC_SEC : RCgenSECS) sol.cuts.insert(an_RC_SEC);
	int after_cuts = sol.cuts.size();
	cout << "RC cuts added = " << after_cuts - b4_cuts << endl;
	sol.RCadded = after_cuts - b4_cuts;

	needed = needed - (after_cuts - b4_cuts);

	cout << " will generate " << needed << " many more using MST." << endl;
	//GENERATE MST_CYCLES OF SIZE size_selected*PRECUT_MULTIPLIER/2 (MANY CUTS). 
	
	set<set<int>> MSTgenSECS = Check_K_MSTCycles_Random(needed, inst, sol);
	
	b4_cuts = sol.cuts.size();
	for (set<int> an_MST_SEC : MSTgenSECS) sol.cuts.insert(an_MST_SEC);
	after_cuts = sol.cuts.size();
	cout << "MST cuts added = " << after_cuts - b4_cuts << endl;
	sol.MSTadded = after_cuts - b4_cuts;
