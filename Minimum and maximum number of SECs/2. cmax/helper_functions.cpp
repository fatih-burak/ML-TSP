...
// Function to generate all combinations of sets from 2 to n
void generateSupersets(const int& numCity, set<set<int>>& subtours_in_k) {
    set<set<int>> supersets;
    vector<set<int>> vec(subtours_in_k.begin(), subtours_in_k.end());
    int n = vec.size();

    // Generate all combinations of size 2 to n
    for (int r = 2; r <= n; ++r) {
        vector<int> indices(n);
        fill(indices.begin(), indices.begin() + r, 1);

        do {
            set<int> unionSet;
            for (int i = 0; i < n; ++i) {
                if (indices[i]) {
                    unionSet.insert(vec[i].begin(), vec[i].end());
                }
            }
            if (unionSet.size() < numCity) supersets.insert(unionSet);
        } while (prev_permutation(indices.begin(), indices.end()));
    }
    // Merge supersets into subtours_in_k
    subtours_in_k.insert(supersets.begin(), supersets.end());
}

void extractUniqueSubsets_And_ReadSubsetsPerSol(Instance& inst, const string& subsetLoc) {
    ifstream infile(subsetLoc);
    string line;

    while (getline(infile, line)) {
        istringstream linestream(line);
        string segment;
        set<set<int>> subtours_in_k; //subtours in solution k

        // Skip the first value (before the first '|')
        getline(linestream, segment, '|'); // discard first column

        // Process the rest of the line
        while (getline(linestream, segment, '|')) {
            istringstream numstream(segment);
            int number;
            set<int> segmentSet;

            while (numstream >> number) {
                //cout << number << " ";
                segmentSet.insert(number);
            }
            //cout << endl;

            if (!segmentSet.empty() && segmentSet.size() < inst.nbCity) { //add only the ones that are not the optimal solution of the instance
                inst.uniqueSubsets.insert(segmentSet);
                subtours_in_k.insert(segmentSet);
            }
        }
        if (!subtours_in_k.empty()) {
            inst.C_of_k.push_back(subtours_in_k);
            generateSupersets(inst.nbCity, subtours_in_k);
            inst.U_of_k.push_back(subtours_in_k);
        }
    }

    /*
    // Output all unique sets
    int idx = 1;
    for (const auto& s : inst.uniqueSubsets) {
        cout << "Set " << idx++ << ": { ";
        for (int val : s) {
            cout << val << " ";
        }
...
void trim(string& s) { // to trim the strings white spaces from the left and right
...
vector<int> setDifference(const vector<int>& vertices, const vector<int>& cycle) {
    /// <summary>
    /// Eeturns the set difference of vertices\cycle
    /// </summary>
    vector<int> result;
    ...
    set_difference(sorted_vertices.begin(), sorted_vertices.end(),
        sorted_cycle.begin(), sorted_cycle.end(),
...
Instance readInstance(const string& filePath, const string& allSolsPath) {
    ...
    extractUniqueSubsets_And_ReadSubsetsPerSol(instance, allSolsPath);
...
void printInfo(const string& pathAndFileout, const Instance& inst, const Solution& sol) {
    ...
    file << fixed << setprecision(2) << inst.name << "\t" << sol.opt << "\t" << inst.nbCity << "\t" << sol.optimalObjVal << "\t" << sol.timeT << "\t" << sol.numSol << endl;
    file.close();
}
