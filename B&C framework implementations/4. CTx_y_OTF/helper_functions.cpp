...
Instance readInstance(const string& filePath) {
    ...
    int idx = 0;
    for (int i = 0; i < dimension; ++i) {
        //cout << i << ": " << instance.xCoord[i] << ", " << instance.yCoord[i] << endl;
        for (int j = 0; j < dimension; ++j) {
...
void printInfo(const string& pathAndFileout, const Instance& inst, const Solution& sol) {
    ...
    file << fixed << setprecision(2) << inst.name << "\t" << sol.opt << "\t" << sol.timeT << "\t" << sol.iter << "\t" << sol.cuts.size() << "\t" << sol.optimalObjVal << "\t" << sol.LB << endl;
    file.close();
}
