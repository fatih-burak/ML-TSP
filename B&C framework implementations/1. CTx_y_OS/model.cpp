...
InterSol ILP(Instance& inst, Solution& sol, int cutType) {
	...
	for (const vector<int>& cycle : sol.cuts) {
	...
	if (model.get(GRB_IntAttr_Status) == GRB_OPTIMAL) {
		for (int j = 0; j < inst.nbCity; j++) {
			for (int i = 0; i < j; i++) {
				if (x[i][j].get(GRB_DoubleAttr_X) > 0.95) {
					inter_sol.sol_edges.push_back({ i,j });
				}
			}
		}
		inter_sol.objVal = model.get(GRB_DoubleAttr_ObjVal);
	}
	else if (model.get(GRB_IntAttr_Status) == GRB_TIME_LIMIT) {
		inter_sol.isTimeLimitHit = true;
		cout << "Time limit!" << endl;
		inter_sol.objVal = ceil(model.get(GRB_DoubleAttr_ObjBound) - EPSILON);
	}
	else {
...
std::ofstream out(inst.cutFolder+inst.name+".txt");
	
	while (true) {
		cout << "Iteration = " << sol.iter + 1 << " and total nb of cuts added = " << sol.cuts.size() << endl;
		int old_no = sol.cuts.size();
		// Call the ILP model
		InterSol iSol = ILP(inst, sol, 1); //last number indicates, cutType: 1->SEC Type 3, 2->SEC Type 5, 3->Both
		sol.LB = iSol.objVal;
		cout << "Current lower bound is = " << sol.LB << endl;

		//If the time limit is reached, exit.
		if (iSol.isTimeLimitHit) break;

		//Check for cycles
		iSol.perc = 1.0; //include the SECs having x% of the size of total vertices.
		iSol.isShortest = true; // true: shortest cut is added
		vector<vector<int>> new_cuts = findCuts(iSol);
		//for (auto content : iSol.shortest_cycle) cout << content << endl;
		
		std::sort(iSol.shortest_cycle.begin(), iSol.shortest_cycle.end());
		for (const auto& content : iSol.shortest_cycle) {
    			out << content << " ";
		}
		out << std::endl; // optional newline at the end
		


		if (new_cuts.empty()) { 
			cout << "Optimal solution found!" << endl;
			sol.optimalObjVal = iSol.objVal;
			sol.opt = 1; break; 
		}
