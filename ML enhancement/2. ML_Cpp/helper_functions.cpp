...
int attDistance(double x1, double y1, double x2, double y2) {
    // Computes ps. Euclidean distance used in ATT instances (approximates real-world travel costs better).
    double dx = x1 - x2;
    double dy = y1 - y2;
...
void printInfo(const string& pathAndFileout, const Instance& inst, const Solution& sol) {
    ...
    cout << sol.LB << endl;
    file << fixed << setprecision(2) << inst.name << "\t" << sol.opt << "\t" << sol.timeT << "\t" << sol.iter << "\t" << sol.cuts.size() << "\t" << sol.eliminatedSECs << "\t" << inst.precuts.size() << "\t" << sol.tNN_pipeline  << "\t" << sol.optimalObjVal << "\t" << sol.LB << endl;
    file.close();
}
